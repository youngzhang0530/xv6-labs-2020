...
/*
 * create a direct-map page table for the process's kernel.
 */
...
// Switch h/w page table register to the proc's kernel page table,
// and enable paging.
...
// Given a process's kernel page table, copy
// its memory into a user page table.
// Copies both the page table and the
// physical memory.
void vmcopy(pagetable_t upgtbl, pagetable_t kpgtbl, uint64 lo, uint64 hi)
{
  for (uint64 i = lo; i < hi; i += PGSIZE)
  {
...
static void print(pagetable_t pgtbl, int level)
{
  for (int i = 0; i < 512; i++)
  {
    pte_t pte = pgtbl[i];
